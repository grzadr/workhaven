...
docker run --name ${CONTAINER_NAME} --rm -it "grzadr/workhaven:${1:-latest}" \
  /bin/bash -c "cat <(conda list --json | grep -Pvae '[\[\]]') <(condaup --dry-run --json | grep -Pzo -e '\"LINK\"\:\s+\[([^\]]+)\]' | head -n -1 | tail -n +2 )" | \
  grep -v -e '^\/bin\/bash' | \
  awk '
  BEGIN{
    RS="\\s+},\\s+";
